...
export function LeadStatusDictionarySection() {
  const t = useT()
  ...
  const errorLoad = t('insurance_desk.config.leadStatus.error.load', 'Failed to load lead status dictionary.')
  const errorSave = t('insurance_desk.config.leadStatus.error.save', 'Failed to save.')
  const successSave = t('insurance_desk.config.leadStatus.success.save', 'Saved.')
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      ...
      console.error('lead status dictionary load', err)
  ...
  const persist = React.useCallback(
    async (next: StatusPayload) => {
      ...
      try {
        ...
        console.error('lead status dictionary save', err)
  ...
  const columns = React.useMemo<ColumnDef<StatusRow>[]>(
    () => [
      {
        id: 'icon',
        header: () => <span className="sr-only">{t('insurance_desk.config.leadStatus.columns.icon', 'Icon')}</span>,
        enableSorting: false,
      ...
      {
        accessorKey: 'value',
        header: t('insurance_desk.config.leadStatus.columns.value', 'Value'),
        cell: ({ getValue }) => <span className="font-mono text-sm">{String(getValue() ?? '')}</span>,
      },
      {
        accessorKey: 'label',
        header: t('insurance_desk.config.leadStatus.columns.name', 'Name'),
      },
      {
        id: 'color',
        header: t('insurance_desk.config.leadStatus.columns.color', 'Color'),
        enableSorting: false,
        cell: ({ row }) => {
  ...
  const deleteConfirm = t('insurance_desk.config.leadStatus.deleteConfirm', 'Delete status "{{label}}"?')
  ...
  const submitDialog = React.useCallback(async () => {
    ...
    if (!valueTrim.length || !labelTrim.length) {
      flash(t('insurance_desk.config.leadStatus.errors.required', 'Value and name are required.'), 'error')
      return
    }
    ...
    if (dialog.mode === 'create') {
      if (payload.entries.some((e) => e.value === normalized.value)) {
        flash(t('insurance_desk.config.leadStatus.errors.duplicate', 'This value already exists.'), 'error')
        return
      }
    ...
    <section className="rounded border bg-card text-card-foreground shadow-sm">
      <div className="space-y-1 border-b px-6 py-4">
        <h2 className="text-lg font-medium">
          {t('insurance_desk.config.leadStatus.heading', 'Inquiry status')}
        </h2>
        <p className="text-sm text-muted-foreground">
          {t(
            'insurance_desk.config.leadStatus.description',
            'Labels (and optional icon/color) for inquiry statuses on lists; values are assigned by the system, not edited manually on forms.',
          )}
        </p>
      </div>
      <div className="px-2 py-4 sm:px-4">
        <DataTable<StatusRow>
          title={t('insurance_desk.config.leadStatus.tableTitle', 'Inquiry status dictionary')}
          actions={
          ...
          emptyState={
            <p className="py-10 text-center text-sm text-muted-foreground">
              {t('insurance_desk.config.leadStatus.empty', 'No status entries yet.')}
            </p>
          }
          pagination={{
            page,
          ...
          {draft ? (
            <div className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="ps-value">{t('insurance_desk.config.leadStatus.form.value', 'Value *')}</Label>
                <input
                  id="ps-value"
              ...
              <div className="space-y-2">
                <Label htmlFor="ps-label">{t('insurance_desk.config.leadStatus.form.name', 'Name *')}</Label>
                <input
                  id="ps-label"
