...
export function PolicyStatusDictionarySection() {
  const t = useT()
  ...
  const errorLoad = t('insurance_desk.config.policyStatus.error.load', 'Failed to load policy status dictionary.')
  const errorSave = t('insurance_desk.config.policyStatus.error.save', 'Failed to save.')
  const successSave = t('insurance_desk.config.policyStatus.success.save', 'Saved.')
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      ...
      console.error('policy status dictionary load', err)
  ...
  const persist = React.useCallback(
    async (next: StatusPayload) => {
      ...
      try {
        ...
        console.error('policy status dictionary save', err)
  ...
  const columns = React.useMemo<ColumnDef<StatusRow>[]>(
    () => [
      {
        id: 'icon',
        header: () => <span className="sr-only">{t('insurance_desk.config.policyStatus.columns.icon', 'Icon')}</span>,
        enableSorting: false,
      ...
      {
        accessorKey: 'value',
        header: t('insurance_desk.config.policyStatus.columns.value', 'Value'),
        cell: ({ getValue }) => <span className="font-mono text-sm">{String(getValue() ?? '')}</span>,
      },
      {
        accessorKey: 'label',
        header: t('insurance_desk.config.policyStatus.columns.name', 'Name'),
      },
      {
        id: 'color',
        header: t('insurance_desk.config.policyStatus.columns.color', 'Color'),
        enableSorting: false,
        cell: ({ row }) => {
  ...
  const deleteConfirm = t('insurance_desk.config.policyStatus.deleteConfirm', 'Delete status "{{label}}"?')
  ...
  const submitDialog = React.useCallback(async () => {
    ...
    if (!valueTrim.length || !labelTrim.length) {
      flash(t('insurance_desk.config.policyStatus.errors.required', 'Value and name are required.'), 'error')
      return
    }
    ...
    if (dialog.mode === 'create') {
      if (payload.entries.some((e) => e.value === normalized.value)) {
        flash(t('insurance_desk.config.policyStatus.errors.duplicate', 'This value already exists.'), 'error')
        return
      }
    ...
    <section className="rounded border bg-card text-card-foreground shadow-sm">
      <div className="space-y-1 border-b px-6 py-4">
        <h2 className="text-lg font-medium">
          {t('insurance_desk.config.policyStatus.heading', 'Policy status')}
        </h2>
        <p className="text-sm text-muted-foreground">
          {t(
            'insurance_desk.config.policyStatus.description',
            'Defines policy statuses for forms (stored value, display name, icon, and color).',
          )}
        </p>
      </div>
      <div className="px-2 py-4 sm:px-4">
        <DataTable<StatusRow>
          title={t('insurance_desk.config.policyStatus.tableTitle', 'Status dictionary')}
          actions={
          ...
          emptyState={
            <p className="py-10 text-center text-sm text-muted-foreground">
              {t('insurance_desk.config.policyStatus.empty', 'No status entries yet.')}
            </p>
          }
          pagination={{
            page,
          ...
          {draft ? (
            <div className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="ps-value">{t('insurance_desk.config.policyStatus.form.value', 'Value *')}</Label>
                <input
                  id="ps-value"
              ...
              <div className="space-y-2">
                <Label htmlFor="ps-label">{t('insurance_desk.config.policyStatus.form.name', 'Name *')}</Label>
                <input
                  id="ps-label"
