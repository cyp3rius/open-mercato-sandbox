...
import { formatMoneyDisplay, formatPercentDisplay } from '@open-mercato/shared/lib/numeric'
...
import type { DriverSettlementDetail } from '../../../../lib/driverSettlements'
import { formatMatchedPayoutTierRange } from '../../../../components/SettlementBasicsPanel'
...
function SummaryRow({ label, value, emphasized = false }: { label: string; value: string; emphasized?: boolean }) {
  return (
    <div className={`flex items-baseline justify-between gap-3 border-b border-[#F1F1F4] py-2.5 last:border-b-0 ${emphasized ? 'pt-3' : ''}`}>
      <span className={emphasized ? 'font-medium text-[#071437]' : driverMutedTextClass}>{label}</span>
      <span className={`tabular-nums ${emphasized ? 'text-base font-semibold text-[#071437]' : 'font-medium text-[#071437]'}`}>
        {value}
      </span>
    </div>
  )
...
export default function DriverSettlementDetailPage({ params }: { params?: { id?: string } }) {
  const t = useT()
  ...
  const tierRange =
    row?.payoutMeta?.mode === 'tiered' && matchedRecord
      ? formatMatchedPayoutTierRange(matchedRecord.fromAmount, matchedRecord.toAmount)
      : ''

  return (
    <DriverShell title={t('taxi_fleet.driverApp.settlements.detailTitle', 'Settlement')}>
          ...
          <>
            <div className={driverCardClass}>
              <div className="flex items-start justify-between gap-3">
                <div>
                  <div className={driverSectionTitleClass}>{formatWeekRange(row.weekStart)}</div>
                  <div className="mt-2">
              ...
              {row.payoutPercent ? (
                <p className={`mt-3 text-sm ${driverMutedTextClass}`}>
                  {t('taxi_fleet.settlements.payoutPercent', 'Payout percent')}:{' '}
                  <span className="font-medium text-[#071437]">{formatPercentDisplay(row.payoutPercent)}</span>
            ...
            <div className={driverCardClass}>
              ...
              <div className="mt-1">
                ...
                {Number(row.bonusAmount) > 0 ? (
                  <SummaryRow
                    label={t('taxi_fleet.settlements.bonusAmount', 'Bonus')}
                    value={formatMoneyDisplay(row.bonusAmount)}
                  />
                ) : null}
                {Number(row.compensationAmount) > 0 ? (
                  <SummaryRow
                    label={t('taxi_fleet.settlements.compensationAmount', 'Compensation')}
                    value={formatMoneyDisplay(row.compensationAmount)}
                  />
                ) : null}
                <SummaryRow
