...
import type { DriverSettlementListItem } from '../../../lib/driverSettlements'
...
export default function DriverSettlementsPage() {
  const t = useT()
  const [items, setItems] = React.useState<DriverSettlementListItem[]>([])
  ...
  const loadSettlements = React.useCallback(async (nextPage: number) => {
    const params = new URLSearchParams({
      page: String(nextPage),
      pageSize: String(PAGE_SIZE),
    })
    const call = await apiCall<SettlementsListResponse>(`/api/taxi_fleet/driver/settlements?${params}`)
    if (!call.ok) {
  ...
  const goToPage = React.useCallback(
    async (nextPage: number) => {
      setLoaded(false)
      try {
        await loadSettlements(nextPage)
      } catch {
        flash(t('taxi_fleet.driverApp.settlements.loadFailed', 'Could not load settlements.'), 'error')
      } finally {
        setLoaded(true)
