...
import { driverCommunicationRetryRecipientSchema } from '@/modules/taxi_fleet/data/validators'
...
export async function POST(
  ...
  try {
    ...
    const parsed = driverCommunicationRetryRecipientSchema.parse({
      communicationId: id,
      recipientId,
    })
    const commandBus = context.container.resolve('commandBus') as CommandBus
    await commandBus.execute('taxi_fleet.driver_communications.retry_recipient', {
      input: parsed,
      ctx: context,
    })
...
export const openApi = {
  POST: {
    summary: 'Retry failed communication recipient delivery',
    tags: ['Taxi fleet'],
