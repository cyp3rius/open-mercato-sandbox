...
import { driverCommunicationCancelSchema } from '@/modules/taxi_fleet/data/validators'
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const parsed = driverCommunicationCancelSchema.parse({ id })
    const commandBus = context.container.resolve('commandBus') as CommandBus
    await commandBus.execute('taxi_fleet.driver_communications.cancel', { input: parsed, ctx: context })
...
export const openApi = {
  POST: {
    summary: 'Cancel scheduled driver communication',
    tags: ['Taxi fleet'],
