...
import { driverCommunicationSendSchema } from '@/modules/taxi_fleet/data/validators'
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const parsed = driverCommunicationSendSchema.parse({ id })
    const commandBus = context.container.resolve('commandBus') as CommandBus
    await commandBus.execute('taxi_fleet.driver_communications.send', { input: parsed, ctx: context })
...
export const openApi = {
  POST: {
    summary: 'Send driver communication now',
    tags: ['Taxi fleet'],
