...
export async function POST(req: Request) {
  try {
    ...
    const orgId = context.selectedOrganizationId ?? auth?.orgId ?? null
    ...
    if (!recordId || !file) {
      throw new CrudHttpError(400, {
        error: translate('taxi_fleet.financial.attachmentRequired', 'Attachment file is required.'),
      })
    }
    ...
    console.info('[taxi_fleet.financial_entries.receipt] upload accepted', {
      attachmentId: item.id,
...
export const openApi = {
  POST: {
    summary: 'Upload financial-entry receipt and start OCR',
    tags: ['Taxi fleet'],
  },
}
