...
import { tripRejectSchema } from '@/modules/taxi_fleet/data/validators'
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const parsed = tripRejectSchema.parse({ id, ...body })
    const commandBus = context.container.resolve('commandBus') as CommandBus
    const { result } = await commandBus.execute<{ id: string; notes?: string | null }, { tripId: string }>(
      'taxi_fleet.trips.reject',
      { input: parsed, ctx: context },
    )
    return NextResponse.json({ id: result?.tripId ?? id, ok: true })
  } catch (err) {
    if (err instanceof CrudHttpError) return NextResponse.json(err.body, { status: err.status })
    console.error('taxi_fleet.trips.reject failed', err)
...
export const openApi = {
  POST: {
    summary: 'Reject trip',
    tags: ['Taxi fleet'],
    requestBody: { schema: tripRejectSchema },
