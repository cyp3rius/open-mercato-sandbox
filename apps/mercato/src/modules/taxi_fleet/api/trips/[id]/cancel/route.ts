...
import { tripCancelSchema, type TripCancelInput } from '@/modules/taxi_fleet/data/validators'
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const parsed = tripCancelSchema.parse({
      id,
      cancelSource: 'operator',
      reason: body.reason ?? null,
    })
    const commandBus = context.container.resolve('commandBus') as CommandBus
    const { result } = await commandBus.execute<TripCancelInput, { tripId: string }>(
      'taxi_fleet.trips.cancel',
      { input: parsed, ctx: context },
    )
    return NextResponse.json({ id: result?.tripId ?? id, ok: true })
  } catch (err) {
    if (err instanceof CrudHttpError) return NextResponse.json(err.body, { status: err.status })
    console.error('taxi_fleet.trips.cancel failed', err)
...
export const openApi = {
  POST: {
    summary: 'Cancel trip',
    tags: ['Taxi fleet'],
    requestBody: { schema: tripCancelSchema },
