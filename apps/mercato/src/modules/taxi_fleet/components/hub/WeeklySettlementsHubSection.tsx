...
import { useSettlementWeekCountdown } from './useSettlementWeekCountdown'
...
export function WeeklySettlementsHubSection() {
  const t = useT()
  const scopeVersion = useOrganizationScopeVersion()
  const { resolveName } = useFleetDriverDirectory()
  const weekCountdown = useSettlementWeekCountdown()
  ...
  React.useEffect(() => {
    ...
    async function load() {
      ...
      const call = await apiCall<ListResponse>(`/api/taxi_fleet/settlements?${params}`)
      if (cancelled) return
  ...
  return (
    <section className="rounded-lg border bg-card px-4 py-3">
      <div className="flex items-start justify-between gap-3">
        <div>
          <h2 className="text-sm font-semibold">{t('taxi_fleet.settlements.weeklyTitle', 'Weekly settlements')}</h2>
          <p className="mt-1 text-sm text-muted-foreground">
            {t('taxi_fleet.hub.settlementsHelp', 'Weekly control settlements (no payout).')}
          </p>
          {weekCountdown ? (
            <p className="mt-2 text-xs tabular-nums text-muted-foreground">{weekCountdown}</p>
          ) : null}
        </div>
        <Button type="button" variant="outline" size="sm" asChild>
          <Link href={`${TAXI_FLEET_BASE}/settlements-overview/weekly`}>
            {t('taxi_fleet.hub.settlements.open', 'Open weekly settlements')}
      ...
      <div className="mt-4 space-y-2">
        {loading ? (
          ...
          <p className="text-sm text-muted-foreground">
            {t('taxi_fleet.hub.weeklySettlements.empty', 'No weekly settlements yet.')}
          </p>
        ) : (
          rows.map((row) => (
            <Link
              key={row.id}
              href={`${TAXI_FLEET_BASE}/settlements/${encodeURIComponent(row.id)}`}
            ...
            >
              <div className="min-w-0">
                ...
                <div className="mt-1 text-xs text-muted-foreground tabular-nums">{formatWeekRange(row.weekStart)}</div>
              </div>
