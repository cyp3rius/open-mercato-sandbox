...
export function mapTripRowToFormValues(row: {
  ...
  const formValues: TripFormValues = {
    ...
    notes: stripAutoGeneratedTripNotes(row.notes ?? ''),
    status: row.status,
    fromLon: '',
    fromLat: '',
    toLon: '',
...
function buildTripPayloadExtras(values: TripFormValues) {
  ...
  const notes = composeTripNotes(details, values.notes)
...
export function tripFormValuesToUpdatePayload(id: string, values: TripFormValues) {
  ...
  return {
    id,
    teamMemberId: values.teamMemberId?.trim() ? values.teamMemberId : null,
    resourceId: values.resourceId?.trim() ? values.resourceId : null,
