...
/** Auto-generated CRM notes block (inject / form route summary). */
export function isAutoGeneratedTripNotesBlock(block: string): boolean {
  const firstLine = block.trim().split('\n')[0]?.trim() ?? ''
  return firstLine.startsWith('Trasa:')
}

/**
 * Remove previously auto-generated route/trip summary blocks so saves do not
 * prepend the same summary repeatedly onto existing notes.
 */
export function stripAutoGeneratedTripNotes(notes: string): string {
  ...
  return blocks.filter((block) => !isAutoGeneratedTripNotesBlock(block)).join('\n\n').trim()
}
...
export function buildTripRouteNotes(details: TripRequestDetails): string | null {
  ...
  const lines = [
    ...
    `Bagaż podręczny: ${Math.max(0, Number(details.handLuggage) || 0)}`,
    `Bagaż do luku: ${Math.max(0, Number(details.holdLuggage) || 0)}`,
    `Foteliki: ${Math.max(0, Number(details.childSeats) || 0)}`,
    `Podstawki: ${Math.max(0, Number(details.boosterSeats) || 0)}`,
    `Kierowca anglojęzyczny: ${yesNo(details.englishSpeakingDriver)}`,
    `Odbiór z lotniska: ${yesNo(isAirport && details.isAirportPickup)}`,
    `Odbiór z tabliczką: ${yesNo(isAirport && details.meetAndGreet)}`,
    details.flightNumber.trim()
      ? `Numer lotu: ${details.flightNumber.trim()}`
      : isAirport && details.isAirportPickup
        ? 'Numer lotu: —'
        : null,
    details.paymentType
...
export function composeTripNotes(details: TripRequestDetails, existingNotes: string): string | undefined {
  const routeNotes = buildTripRouteNotes(details)
  const manualNotes = stripAutoGeneratedTripNotes(existingNotes)
  const composed = [routeNotes, manualNotes].filter((part) => part && part.length).join('\n\n')
  return composed.length ? composed : undefined
}
