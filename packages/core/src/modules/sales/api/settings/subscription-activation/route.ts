...
import {
  ...
  salesSubscriptionActivationSettingsSchema,
...
import { resolveSubscriptionActivationOrderStatuses } from '../../../lib/subscriptionActivation'
...
export async function GET(req: Request) {
  try {
    ...
    return NextResponse.json({
      subscriptionActivationOrderStatuses: resolveSubscriptionActivationOrderStatuses(
        record?.subscriptionActivationOrderStatuses,
      ),
      orderStatuses,
    })
  } catch (err) {
    ...
    const { translate } = await resolveTranslations()
    console.error('sales.settings.subscription-activation.get failed', err)
    return NextResponse.json(
...
export async function PUT(req: Request) {
  try {
    ...
    const parsed = salesSubscriptionActivationSettingsSchema.parse(scoped)
    ...
    const commandInput = salesSettingsUpsertSchema.parse({
      ...
      subscriptionActivationOrderStatuses: resolveSubscriptionActivationOrderStatuses(
        parsed.subscriptionActivationOrderStatuses,
      ),
    })
    ...
    const result = (
      response as {
        result?: { subscriptionActivationOrderStatuses?: string[] | null }
      }
    ).result
    ...
    return NextResponse.json({
      subscriptionActivationOrderStatuses: resolveSubscriptionActivationOrderStatuses(
        result?.subscriptionActivationOrderStatuses,
      ),
      orderStatuses,
    })
  } catch (err) {
    ...
    const { translate } = await resolveTranslations()
    console.error('sales.settings.subscription-activation.put failed', err)
    return NextResponse.json(
...
const settingsResponseSchema = z.object({
  subscriptionActivationOrderStatuses: z.array(z.string()),
...
export const openApi: OpenApiRouteDoc = {
  tag: 'Sales',
  summary: 'Sales subscription activation settings',
  methods: {
    GET: {
      summary: 'Get order statuses that activate subscription offerings',
      responses: [
        {
          status: 200,
          description: 'Current subscription activation statuses',
    ...
    PUT: {
      summary: 'Update order statuses that activate subscription offerings',
      requestBody: {
        contentType: 'application/json',
        schema: salesSubscriptionActivationSettingsSchema,
      },
      responses: [
        {
          status: 200,
          description: 'Updated subscription activation statuses',
          schema: settingsResponseSchema,
        },
        { status: 401, description: 'Unauthorized', schema: settingsErrorSchema },
