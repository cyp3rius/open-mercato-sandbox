...
export class Migration20260504075647 extends Migration {
  ...
  override async down(): Promise<void> {
    ...
    this.addSql(`create table "playbooks_playbooks" ("id" uuid not null default gen_random_uuid(), "tenant_id" uuid not null, "organization_id" uuid not null, "slug" text not null, "title" text not null, "body" text not null, "context_tags" jsonb not null default '[]', "audience" text not null default 'internal', "version" int4 not null default 1, "published_at" timestamptz(6) null, "is_active" bool not null default true, "created_at" timestamptz(6) not null, "updated_at" timestamptz(6) not null, "deleted_at" timestamptz(6) null, constraint "playbooks_playbooks_pkey" primary key ("id"));`);
    this.addSql(`create index "playbooks_scope_idx" on "playbooks_playbooks" ("tenant_id", "organization_id");`);
    this.addSql(`CREATE UNIQUE INDEX playbooks_slug_scope_uidx ON public.playbooks_playbooks USING btree (organization_id, slug) WHERE (deleted_at IS NULL);`);
