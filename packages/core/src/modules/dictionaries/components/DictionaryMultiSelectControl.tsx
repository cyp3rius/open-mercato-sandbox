...
import { DictionaryEntryMultiSelect } from './DictionaryEntryMultiSelect'
import {
  ensureDictionaryEntries,
  invalidateDictionaryEntries,
} from './hooks/useDictionaryEntries'

type DictionaryMultiSelectControlProps = {
  dictionaryId: string
  value?: string[] | null
  ...
  /** Accessible label for removing one chip (passed through to the multi selector). */
  removeEntryAriaLabel?: string
}

export function DictionaryMultiSelectControl({
  dictionaryId,
  value,
  onChange,
...
}: DictionaryMultiSelectControlProps) {
  const t = useT()
  ...
  React.useEffect(() => {
    ...
    async function evaluateInlineCreate() {
      ...
      try {
        ...
        console.warn('DictionaryMultiSelectControl.inlineCreate check failed', err)
        if (!cancelled) {
  ...
  const labels = React.useMemo(
    () => ({
      placeholder: t('dictionaries.customFields.selector.placeholderMulti', 'Add entries…'),
  ...
  const normalizedValue = Array.isArray(value) ? value.filter((v): v is string => typeof v === 'string') : []
  ...
  return (
    <DictionaryEntryMultiSelect
      ...
      removeEntryAriaLabel={
        removeEntryAriaLabel ?? t('dictionaries.customFields.selector.removeEntry', 'Remove entry')
      }
    />
  )
}
