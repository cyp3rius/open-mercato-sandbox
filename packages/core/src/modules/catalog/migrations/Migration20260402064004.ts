...
export class Migration20260402064004 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`alter table if exists "organizations" drop constraint if exists "organizations_tenant_id_foreign";`);
    ...
    this.addSql(`create table "catalog_service_lines" ("id" uuid not null default gen_random_uuid(), "organization_id" uuid not null, "tenant_id" uuid not null, "code" text not null, "title" text not null, "description" text null, "sort_order" int not null default 0, "is_active" boolean not null default true, "created_at" timestamptz not null, "updated_at" timestamptz not null, "deleted_at" timestamptz null, constraint "catalog_service_lines_pkey" primary key ("id"));`);
    ...
    this.addSql(`alter table "catalog_products" add column if not exists "service_line_id" uuid null;`);
    this.addSql(`
      do $$
      begin
        if not exists (
          select 1 from pg_constraint where conname = 'catalog_products_service_line_id_foreign'
        ) then
    ...
    this.addSql(`alter table "catalog_product_variants" drop column if exists "default_media_id", drop column if exists "default_media_url";`);

    this.addSql(`alter table if exists "catalog_product_variant_relations" drop constraint if exists "catalog_product_variant_relations_unique";`);
    ...
    this.addSql(`alter table if exists "catalog_product_variant_prices" drop constraint if exists "catalog_product_variant_prices_unique";`);
  }

  override async down(): Promise<void> {
    this.addSql(`alter table if exists "catalog_products" drop constraint if exists "catalog_products_service_line_id_foreign";`);
    ...
    this.addSql(`alter table if exists "catalog_product_relations" drop constraint if exists "catalog_product_relations_child_product_id_foreign";`);
    ...
    this.addSql(`alter table if exists "catalog_product_relations" drop constraint if exists "catalog_product_relations_parent_product_id_foreign";`);
    ...
    this.addSql(`alter table if exists "catalog_product_variant_option_values" drop constraint if exists "catalog_product_variant_option_values_variant_id_foreign";`);
    ...
    this.addSql(`drop index if exists "catalog_product_variant_relations_child_product_idx";`);
    ...
    this.addSql(`alter table "catalog_products" drop column "service_line_id", drop column "default_sales_unit", drop column "default_sales_unit_quantity", drop column "uom_rounding_scale", drop column "uom_rounding_mode", drop column "unit_price_enabled", drop column "unit_price_reference_unit", drop column "unit_price_base_quantity";`);
  }
