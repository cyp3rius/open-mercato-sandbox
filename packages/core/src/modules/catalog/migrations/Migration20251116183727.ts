...
export class Migration20251116183727 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`alter table "catalog_products" add column "custom_fieldset_code" text null;`);

    this.addSql(`alter table "catalog_product_variants" drop column "attribute_schema", drop column "attribute_values";`);

    this.addSql(`alter table "catalog_product_variants" add column "custom_fieldset_code" text null;`);

    this.addSql('drop table if exists "catalog_product_attribute_schemas" cascade;')
