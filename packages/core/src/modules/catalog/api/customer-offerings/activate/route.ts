...
export const openApi = {
  tags: ['Catalog'],
  summary: 'Activate a customer product offering',
}
...
export async function POST(req: Request) {
  try {
    ...
    const { result } = await commandBus.execute('catalog.customer_offerings.activate', {
      input: {
    ...
    console.error('catalog.customer_offerings.activate failed', err)
    return NextResponse.json(
      {
        error: translate(
          'catalog.customerOfferings.errors.activate',
          'Failed to activate offering.',
        ),
      },
      { status: 400 },
    )
