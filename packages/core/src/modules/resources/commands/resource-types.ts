...
async function loadResourceTypeSnapshot(em: EntityManager, id: string): Promise<ResourceTypeSnapshot | null> {
  ...
  return {
    ...
    vehicleFinancingEligible: resourceType.vehicleFinancingEligible ?? false,
...
const createResourceTypeCommand: CommandHandler<ResourcesResourceTypeCreateInput, { resourceTypeId: string }> = {
  ...
  async execute(rawInput, ctx) {
    ...
    const record = em.create(ResourcesResourceType, {
      ...
      vehicleFinancingEligible: parsed.vehicleFinancingEligible ?? false,
      createdAt: now,
      updatedAt: now,
    })
    em.persist(record)
...
const updateResourceTypeCommand: CommandHandler<ResourcesResourceTypeUpdateInput, { resourceTypeId: string }> = {
  ...
  async execute(rawInput, ctx) {
    ...
    if (parsed.vehicleFinancingEligible !== undefined) record.vehicleFinancingEligible = parsed.vehicleFinancingEligible
  ...
  undo: async ({ logEntry, ctx }) => {
    ...
    resourceType.vehicleFinancingEligible = before.vehicleFinancingEligible
  ...
  undo: async ({ logEntry, ctx }) => {
    ...
    if (!resourceType) {
      resourceType = em.create(ResourcesResourceType, {
        ...
        vehicleFinancingEligible: before.vehicleFinancingEligible,
        deletedAt: null,
        createdAt: new Date(),
        updatedAt: new Date(),
      })
