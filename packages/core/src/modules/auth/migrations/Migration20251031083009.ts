...
export class Migration20251031083009 extends Migration {
  ...
  async up(): Promise<void> {
    this.addSql(`alter table if exists "roles" drop constraint if exists "${this.oldConstraint}";`)
    this.addSql(`alter table "roles" add constraint "${this.newConstraint}" unique ("tenant_id", "name");`)
  }

  async down(): Promise<void> {
    this.addSql(`alter table if exists "roles" drop constraint if exists "${this.newConstraint}";`)
