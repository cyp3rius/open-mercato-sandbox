...
function processPageFiles(options: {
  ...
  for (const { relPath, fromApp } of files.filter(({ relPath: f }) => !f.endsWith('/page.tsx') && f !== 'page.tsx')) {
    ...
    const baseProps = `pattern: '${routePath || '/'}', requireAuth: (${metaExpr})?.requireAuth, requireRoles: (${metaExpr})?.requireRoles, requireFeatures: (${metaExpr})?.requireFeatures, title: (${metaExpr})?.pageTitle ?? (${metaExpr})?.title, titleKey: (${metaExpr})?.pageTitleKey ?? (${metaExpr})?.titleKey, group: (${metaExpr})?.pageGroup ?? (${metaExpr})?.group, groupKey: (${metaExpr})?.pageGroupKey ?? (${metaExpr})?.groupKey, navFlat: (${metaExpr})?.navFlat`
...
export async function generateModuleRegistry(options: ModuleRegistryOptions): Promise<GeneratorResult> {
  ...
  const messageObjectClientTypeEntries: string[] = []
  const messageObjectClientTypeImports: string[] = []
  ...
  for (const entry of enabled) {
    ...
    // Message object types: message-objects.ts (server, may import ORM-backed previews)
    // message-objects.client.ts when present for browser-safe registration (see messages.client.generated)
    {
      ...
      if (resolved) {
        ...
        const resolvedClient = resolveModuleFile(roots, imps, 'message-objects.client.ts') ?? resolved
        const clientImportName = `MSG_OBJECTS_CLIENT_${toVar(modId)}_${importIdRef.value++}`
        messageObjectClientTypeImports.push(`import * as ${clientImportName} from '${resolvedClient.importPath}'`)
        messageObjectClientTypeEntries.push(
          `{ moduleId: '${modId}', types: ((${clientImportName}.default ?? (${clientImportName} as any).messageObjectTypes ?? (${clientImportName} as any).objectTypes ?? (${clientImportName} as any).types ?? []) as MessageObjectTypeDefinition[]) }`
        )
      }
    }
...
`
  ...
  const messageObjectClientEntriesLiteral = messageObjectClientTypeEntries.join(',\n  ')
  const messageObjectClientImportSection = messageObjectClientTypeImports.join('\n')
...
${messageTypeImportSection ? `\n${messageTypeImportSection}\n` : '\n'}${messageObjectClientImportSection ? `\n${messageObjectClientImportSection}\n` : ''}type MessageTypeEntry = { moduleId: string; types: MessageTypeDefinition[] }
type MessageObjectTypeEntry = { moduleId: string; types: MessageObjectTypeDefinition[] }
...
${messageObjectClientEntriesLiteral ? `  ${messageObjectClientEntriesLiteral}\n` : ''}]
