...
/**
 * Inline field maps so this file has no relative imports to ./entities/*.
 * Post-processors / stale tooling sometimes rewrite those imports to `.js`, which breaks Turbopack
 * because only `index.ts` stubs exist beside the generated registry.
 */
function writeEntityFieldsRegistry(generatedRoot: string, fieldsByEntity: EntityFieldMap): void {
  ...
  const registryEntries =
    entities.length > 0
      ? entities
          .map((slug) => {
            const fields = fieldsByEntity[slug] ?? []
            const inner =
              fields.length > 0
                ? fields
                    .map((f) => `    ${toVar(f)}: '${escapeSingleQuotedJs(f)}'`)
                    .join(',\n')
                : ''
...
// Static registry for entity fields — inlined (no ./entities/* imports; avoids bad .js rewrites).
