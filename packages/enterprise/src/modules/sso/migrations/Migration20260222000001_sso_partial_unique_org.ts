...
export class Migration20260222000001_sso_partial_unique_org extends Migration {
  ...
  override async up(): Promise<void> {
    this.addSql(`alter table if exists "sso_configs" drop constraint if exists "sso_configs_organization_id_unique";`);
  ...
  override async down(): Promise<void> {
    this.addSql(`drop index if exists "sso_configs_organization_id_unique";`);
