...
export class Migration20260225000000_sso_identities_partial_unique extends Migration {
  ...
  override async up(): Promise<void> {
    this.addSql(`alter table if exists "sso_identities" drop constraint if exists "sso_identities_config_user_unique";`);
    this.addSql(`alter table if exists "sso_identities" drop constraint if exists "sso_identities_config_subject_unique";`);
  ...
  override async down(): Promise<void> {
    this.addSql(`drop index if exists "sso_identities_config_user_unique";`);
    this.addSql(`drop index if exists "sso_identities_config_subject_unique";`);
    this.addSql(`drop index if exists "sso_identities_config_external_id_unique";`);
